...
namespace {
  ...
  private:
    ...
    std::vector<int> get_roc_col_row(int x, int y);
  ...
  std::vector<int> SiPixelPhase1BumpBonding::get_roc_col_row(int x, int y) {
    if ( y == 0){
  ...
  module_bin SiPixelPhase1BumpBonding::convert_to_bool(MonitorElement* h) {
	...
	std::vector<int> roc_roc_x_rocy = get_roc_col_row(int(x), int(y));
